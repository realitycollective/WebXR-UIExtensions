...
A windowing / docking / layout / controls library for **WebXR spatial UI** - an engine-free core plus adapters for
[Meta's Immersive Web SDK](https://iwsdk.dev) and [Google XR Blocks](https://github.com/google/xr-blocks) / plain three.js, developer tooling, and deployable demo clients.
...
Every package has a demo client, and all three demos build the **same playground** - the scene is a portable `SceneDescriptor` and its behaviour is wired through the engine-free `onPanelReady` contract, so the IWSDK, XR Blocks and plain-three.js pipelines run identical UX from identical data. Each adapter re-exports the core wholesale, so apps depend on exactly one package:
